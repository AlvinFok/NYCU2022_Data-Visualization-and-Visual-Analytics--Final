...
filter_city = []
with open('elbase_P1.csv', newline='', encoding="utf-8") as csvfile:
    rows = csv.reader(csvfile)
    for row in rows:
...
canditates = {}
with open('elcand_P1.csv', newline='', encoding="utf-8") as csvfile:
    rows = csv.reader(csvfile)
    for row in rows:
        if row[-1] != 'Y':
...
tks_by_nation = []
tks_by_city = []
with open('elctks_P1.csv', newline='', encoding="utf-8") as csvfile:
    rows = csv.reader(csvfile)
    for row in rows:
...
with open('../../cleaned/president/2016/by_nation.csv', 'w', newline='', encoding="utf-8") as csvfile:
...
with open('../../cleaned/president/2016/by_city.csv', 'w', newline='', encoding="utf-8") as csvfile:
    writer = csv.writer(csvfile)
